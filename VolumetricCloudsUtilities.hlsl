// HanPi Volume Cloud — GitHub 阅读版
// 摘自 Unity HDRP VolumetricCloudsUtilities.hlsl 中的 HanPi 改动（非可编译单元）。
//
// 标记说明：
//   [HDRP]  Unity 官方基础设施 — 此处仅保留桩代码/结构体，便于顺读 HanPi 逻辑
//   [HanPi]  HanPi 体积云系统实现 — 完整保留
//
// 建议阅读顺序：
//   1. HanPi 参数与宏（HPVolumeCloudRenderDriver 注入）
//   2. EvaluateCloudProperties / EvaluateHighCloudDensity — 密度
//   3. EvaluateSunLuminance — 光步 + hp_phi_fwd
//   4. EvaluateCloud — 散射积分（scatterSource + hp_phi_fwd + AO）
//   5. HPTraceVolumetricRay — 主 trace（低云分层步进 + 高空云合成）
//
// 完整 HDRP 源码：Packages/com.unity.render-pipelines.high-definition/
//   Runtime/Lighting/VolumetricClouds/VolumetricCloudsUtilities.hlsl

#ifndef GITHUB_HANPI_CLOUD_READING_H
#define GITHUB_HANPI_CLOUD_READING_H
...
// 常用内建/工具函数 — 实际由 HDRP ShaderLibrary 提供
float PositivePow(float x, float p) { return pow(max(x, 0.0), p); }
float3 PositivePow(float3 x, float p) { return pow(max(x, 0.0), p); }
float3 SampleSH9(StructuredBuffer<float4> buf, float3 dir) { return float3(0, 0, 0); }
float GetCurrentExposureMultiplier() { return 1.0; }
#define ZERO_INITIALIZE(type, name) type name = (type)0

// ── [HDRP] 全局纹理/常量（HanPi 与官方共用）────────────────────────────────
...
#define SAMPLE_TEXTURE2D_LOD(tex, samp, uv, mip) tex.SampleLevel(samp, uv, mip)
#define SAMPLE_TEXTURE3D_LOD(tex, samp, uvw, mip) tex.SampleLevel(samp, uvw, mip)
...
// Base noise channels: R=PerlinWorley, G=Worley
// Weather map layout:
...
float  _HP_ScWorleyStrength;   // Sc 整体权重：>0 激活，1=完全层积云
float  _HP_ScHeightScale;      // Sc 层高度压缩比：积云廓形被压进 slab 底部此比例范围（0.05=极薄，0.5=半高）
float  _HP_ScDetailStr;        // Sc 独立侵蚀强度
...
// ── 低云运行时 Coverage 调整（Cover / Height 双路）──
...
TEXTURE2D(_HP_HiWispTex);      // 高空云云絮纹理（R 通道；留空时 RenderDriver 传入 whiteTexture）
...
// ── 低密度散射源 ────────────────────────────────────────────────────────────
// 稀薄边缘的单次散射源由本步 scatterOD 决定：amount = pow(saturate(1 - exp(-scatterOD / ODScale)), CurvePow)。
// 视线透射率仍用完整 extinction；hp_phi_fwd 单独用 transmittance 注入。
// _HP_ScatterSourceODScale  : 散射 OD 参考尺度（exp 分母）；越小边缘源强建立越慢，建议 0.01~0.3
// _HP_ScatterSourceCurvePow : 源强曲线指数（>1 边缘更硬，1=线性，<1 更软）
float _HP_ScatterSourceODScale;
float _HP_ScatterSourceCurvePow;
...
// 多重散射三参数（Hillaire 2020）：可分别调节穿透 / 亮度 / 各向异性。
...
// ── hp_phi_fwd 物理漫射场（HpPhiFwd Diffuse Field） ─────────────────────────────────
...
// _HP_HpHpPhiFwd_Intensity  : 整体强度（0=禁用，建议 0.1~2.0）
// _HP_HpHpPhiFwd_DepthPow   : 漫射场深度衰减指数。修正 hp_phi_fwd 在云底偏大的问题（物理上漫射场从云顶向下衰减）。
//                         saturate(localHeight + Bias)^DepthPow：DepthPow=1线性，0=禁用。建议 0.5~2.0。
// _HP_HpHpPhiFwd_DepthBias  : 深度曲线垂直偏移。>0 底面保留更多漫射光（不降到0）；<0 过渡区上移，底面更暗。建议 -0.3~0.5。
// _HP_HpHpPhiFwd_BoundaryConfidence : 2D 高度差分边界受光置信度强度。0=禁用，1=全量使用 wrap 边界受光。
// _HP_HpHpPhiFwd_MSBuildScale : 各向同性 MS 建立速度。越大越快从边界方向性散射 lerp 到漫射源。
// _HP_HpHpPhiFwd_Compress : hp_phi_fwd 最终软饱和压缩。0=线性不压缩；越大越早饱和。
float _HP_HpHpPhiFwd_Intensity;
float _HP_HpHpPhiFwd_DepthPow;
float _HP_HpHpPhiFwd_DepthBias;
float _HP_HpHpPhiFwd_BoundaryConfidence;
float _HP_HpHpPhiFwd_MSBuildScale;
float _HP_HpHpPhiFwd_Compress;

// 用低云天气图重建一个便宜的云顶高度代理，供 hp_phi_fwd 边界受光判断使用。
// 这里只关心“最近相关边界是否面向太阳”，不参与密度本身的精确评估。
float HP_EvaluateHpHpPhiFwdTopHeightProxy(float2 worldXZ)
{
...
float HP_EvaluateHpHpPhiFwdBoundaryLight(float3 positionWS, float3 sunDirection)
{
    float sampleStep = clamp(_HP_WeatherMapWorldSize * 0.001, 25.0, 200.0);

    float hL = HP_EvaluateHpHpPhiFwdTopHeightProxy(positionWS.xz - float2(sampleStep, 0.0));
    float hR = HP_EvaluateHpHpPhiFwdTopHeightProxy(positionWS.xz + float2(sampleStep, 0.0));
    float hD = HP_EvaluateHpHpPhiFwdTopHeightProxy(positionWS.xz - float2(0.0, sampleStep));
    float hU = HP_EvaluateHpHpPhiFwdTopHeightProxy(positionWS.xz + float2(0.0, sampleStep));
    ...
    return lerp(1.0, boundaryLitWarp, saturate(_HP_HpHpPhiFwd_BoundaryConfidence));
}
...
struct CloudProperties
{
    ...
    float height;       // slab 归一化高度 0~1
    float localHeight;  // HanPi：云体 profile 局部高度，供 AO / hp_phi_fwd 使用
    float sigmaT;
};
...
struct RayMarchRange { float start; float distance; };

// ── [HDRP] 几何与工具函数（官方，桩/精简）──────────────────────────────────
...
// EvaluateEnvironmentLighting [HDRP + HanPi 改动]：
//   官方框架；下方保留 HanPi 注入的环境光倍增与 MS 偏心率参数。
EnvironmentLighting EvaluateEnvironmentLighting(CloudRay ray, float3 entryWS, float3 exitWS)
{
    EnvironmentLighting lighting;
    lighting.sunDirection = _SunDirection;
    lighting.sunColor0 = _SunLightColor * GetCurrentExposureMultiplier();
    ...
    // [HDRP] 太阳大气衰减 EvaluateSunColorAttenuation(...) 省略
    lighting.phaseFunction[0] = HenyeyGreenstein(lighting.cosAngle,  _HP_ForwardEccentricity  * PositivePow(_HP_MS_Eccentricity, 0))
                              + HenyeyGreenstein(lighting.cosAngle, -_HP_BackwardEccentricity * PositivePow(_HP_MS_Eccentricity, 0));
    #if NUM_MULTI_SCATTERING_OCTAVES >= 2
    lighting.phaseFunction[1] = HenyeyGreenstein(lighting.cosAngle,  _HP_ForwardEccentricity  * PositivePow(_HP_MS_Eccentricity, 1))
                              + HenyeyGreenstein(lighting.cosAngle, -_HP_BackwardEccentricity * PositivePow(_HP_MS_Eccentricity, 1));
    #endif
    #if NUM_MULTI_SCATTERING_OCTAVES >= 3
    lighting.phaseFunction[2] = HenyeyGreenstein(lighting.cosAngle,  _HP_ForwardEccentricity  * PositivePow(_HP_MS_Eccentricity, 2))
                              + HenyeyGreenstein(lighting.cosAngle, -_HP_BackwardEccentricity * PositivePow(_HP_MS_Eccentricity, 2));
    #endif
    return lighting;
}

bool GetCloudVolumeIntersection(float3 originWS, float3 dir, float insideClouds, float toEarthCenter, out RayMarchRange r)
{
    // [HDRP] 球壳云层求交；阅读版省略完整实现
    ZERO_INITIALIZE(RayMarchRange, r);
    r.start = 0;
    r.distance = 10000;
    return true;
}

bool PointInsideCloudVolume(float3 positionPS) { return true; }
float EvaluateNormalizedCloudHeight(float3 positionPS) { return 0.5; }

// 以下官方函数在本阅读版中省略（HanPi 核心逻辑不依赖其细节）：
//   EvaluateSunColorAttenuation, GetCloudViewDirWS, ConvertCloudDepth,
//   ConvertObliqueDepthToNonOblique, GetCloudCoverageData,
//   DensityFadeValue, ComputeCheckerBoardIndex, EvaluateFinalTransmittance

// ── [HanPi] 密度评估 ────────────────────────────────────────────────────────
// ── HanPi VolumeCloud density algorithm ──────────────────────────────────────
// Billowy / Wispy 双层侵蚀密度评估。
// noiseMipOffset / erosionMipOffset：3D 噪声 LOD（距离 / 光步）。
// simpleMode=true：跳过 erosion 采样，用于大步密度探测。
// simpleMode=false：完整计算，含 Billowy/Wispy detail erosion。
                            ...
                            bool simpleMode, out CloudProperties properties)
{
    ...
    // ── 运行时 Coverage 双路调整 ────────────────────────────────────────────
    // weather.r 先统一读出，再各自经 pow(contrast)×intensity 后用于不同计算路径。
    ...
    // ── Sc 空隙塑形（ScWorleyStrength > 0 时激活）─────
...
float3 EvaluateSunLuminance(float3 positionWS, float currentLocalHeight, float3 sunDirection, float3 sunColor, PHASE_FUNCTION_STRUCTURE phaseFunction, out float lightExtinctionOD, out float hpHpPhiFwd)
{
    ...
    if (GetCloudVolumeIntersection_Light(positionWS, sunDirection, totalLightDistance))
    {
        ...
        float kappaODSum    = 0.0f; // hp_phi_fwd：观测点→当前步入口的 ∫κ ds 累积
        float T_cum         = 1.0f;
        float curWidth      = w0;
        float cumDist       = 0.0f;
        float hpHpPhiFwdBottomHeight = currentLocalHeight + _HP_HpHpPhiFwd_DepthBias;
        float hpHpPhiFwdColumnThickness = HP_EvaluateHpHpPhiFwdTopHeightProxy(positionWS.xz);
        float hpHpPhiFwdBottomSoftHeight = max(_HP_BottomSmoothHeight * lerp(1.0, 4.0, hpHpPhiFwdColumnThickness), 0.001);
        float srcBottomConfidence = (_HP_HpHpPhiFwd_DepthPow > 0.0)
            ? 1.0 - exp(-max(hpHpPhiFwdBottomHeight, 0.0) / hpHpPhiFwdBottomSoftHeight * _HP_HpHpPhiFwd_DepthPow)
            : 1.0;
        float srcConfidence = HP_EvaluateHpHpPhiFwdBoundaryLight(positionWS, sunDirection) * srcBottomConfidence;
        [loop]
        for (int j = 0; j < numSteps; j++)
        {
            ...
            // ── hp_phi_fwd 1D 格林函数：φ += T_src·(Q/D)·exp(−∫κ ds)·(1/r) ──
            ...
            float msBuild      = 1.0f - exp(-(extinctionSum + localOD * 0.5f) * max(_HP_HpHpPhiFwd_MSBuildScale, 0.0f));
            float invR         = 1.0f / max(dist, stepWidth * 0.5f); // 几何衰减 1/r_j，r_j = 到步中心距离
            hpHpPhiFwd            += (T_cum) * qSrc * invD * srcConfidence * msBuild * perSrcExp * invR;
        ...
        lightExtinctionOD = extinctionSum;

        float3 extinction = _ScatteringTint.xyz * extinctionSum;
                ...
                inout VolumetricRayResult volumetricRay)
{
    ...
    float hpHpPhiFwd;
    float3 totalLuminance = EvaluateSunLuminance(currentPositionWS, cloudProperties.localHeight, envLighting.sunDirection, sunColor, envLighting.phaseFunction, lightExtinctionOD, hpHpPhiFwd);

    // ── 方向性/环境散射源积分 ────────────────────────────────────────────────
    // 低密度边缘的单次散射源由本步散射 OD 决定，而不是通过缩短 extinction 步长近似。
    // 视线透射率仍用完整 extinction 保持云体不透明度；hp_phi_fwd 单独在下方用 transmittance 注入。
    float scatterOD = extinction * HP_PHIFWD_OMEGA0 * stepSize;
    float scatterSourceAmount = 1.0 - exp(-scatterOD / max(_HP_ScatterSourceODScale, 0.001));
    scatterSourceAmount = PositivePow(saturate(scatterSourceAmount), max(_HP_ScatterSourceCurvePow, 0.01));

    // 方向性散射：按本步散射 OD 积分出的有效源强
    float3 integScatt = totalLuminance * scatterSourceAmount;

    // ── hp_phi_fwd 物理漫射场加性散射（HpPhiFwd Diffuse Field） ───────────────────
    // hpHpPhiFwd 是沿光线步进积累的各向同性漫射场能量密度（无量纲 OD 加权积分）。
    // 与 Hillaire 乘性 MS 正交：这是独立的加性散射项，代表真实扩散的漫射辐射场。
    // 使用完整 transmittance 做步长积分（不受 scatterSource 曲线影响）。
    //
    // 底部/边界置信度已在 EvaluateSunLuminance 的源项中应用；这里仅将积分结果注入视线散射。
    if (_HP_HpHpPhiFwd_Intensity > 0.0)
    {
        float hpHpPhiFwdScalar = hpHpPhiFwd * _HP_HpHpPhiFwd_Intensity;
        float hpHpPhiFwdMapped = (_HP_HpHpPhiFwd_Compress > 0.0)
            ? (1.0 - exp(-hpHpPhiFwdScalar * _HP_HpHpPhiFwd_Compress)) / _HP_HpHpPhiFwd_Compress
            : hpHpPhiFwdScalar;
        float3 hpHpPhiFwdLuminance = hpHpPhiFwdMapped * sunColor;
        integScatt += hpHpPhiFwdLuminance - hpHpPhiFwdLuminance * transmittance;
    }
    ...
    // _HP_AOUpwardScale：1.0=物理值，>1 加强遮蔽，<1 减弱。
    ...
    integScatt                += ambientTerm * scatterSourceAmount;
...
//
// 低仰角 / 地平线方向 totalDist 过大时，均匀大步会跨过近处薄云。
// 采用距离自适应 stepLarge + 命中云体后 stepSmall 精细积分（参考 HZD / UE5 VolumetricCloud）。
// ─────────────────────────────────────────────────────────────────────────────
    ...
    while (dist < totalDist && iterCount < maxIter)
    {
        ...
        if (simpleProps.density > HI_CLOUD_DENSITY_THRESHOLD)
        {
            ...
            if (fullProps.density > HI_CLOUD_DENSITY_THRESHOLD)
            {
                ...
                // #define HP_DEBUG_LOCAL_HEIGHT
                // #define HP_DEBUG_SLAB_HEIGHT
...
#endif // GITHUB_HANPI_CLOUD_READING_H
